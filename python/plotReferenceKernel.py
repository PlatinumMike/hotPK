...
resKz = 1000
kzMax = 4
kz = np.linspace(-kzMax,kzMax,resKz) #already normalized, so times 2 \rho_T, and distances z are divided by 2\rho_T.
...
for harmonicIndex in range(NHarmonics):
    ...
    zRange = np.concatenate((-np.flip(tmp),tmp))*zMax #has no zero, but peaked near the origin to resolve spike better.
    ...
    for i in range(7):
        for j in range(resR):
            HValues[i, j] = hfg.getH(rRange[j], i, harmonic, jacobian=True) #we might as well already include factor xi already here.
