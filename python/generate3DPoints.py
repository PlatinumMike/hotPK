...
def getPoints(
    ...
    for index in range(7):
        for harmonic in range(-nMax, nMax + 1):
            for ix in range(resolutionX):
                for iy in range(resolutionY):
                    HValues[index, harmonic + nMax, ix, iy] = float(
                        hfg.getH(radius[ix, iy], index, harmonic, False)
                    )
    print("Done, took ", time.time() - startTime, " seconds")
        ...
        for iy in range(resolutionY):
            ...
            for harmonicIndex in range(NHarmonics):
                ...
                D4 += (
                    ...
                    * signCyc
                    * cOverVT
                    * HValues[5, harmonicIndex, ix, iy]
                    * S1numpy[harmonicIndex, :]
                )
                D5 += (
                    np.sqrt(2)
                    * signCyc
                    * cOverVT
                    * HValues[6, harmonicIndex, ix, iy]
                    * S1numpy[harmonicIndex, :]
                )
                ...
                R4 += (
                    ...
                    * signCyc
                    * HValues[5, harmonicIndex, ix, iy]
                    * S2numpy[harmonicIndex, :]
                )
                R5 += (
                    -0.5 * np.sqrt(2) * signCyc * HValues[6, harmonicIndex, ix, iy] * S2numpy[harmonicIndex, :]
                )
