...
def getS2(eps,x):
    return eps*x*mp.meijerg([[],[]],[[0,0],[-1/2]],x*x) + mp.sign(x)*1.0j*(2*mp.sqrt(mp.pi)*abs(x)*mp.hyper([],[1,3/2],x*x)-mp.hyper([],[1/2,1/2],x*x))

def getS3(eps,x):
    return -1.0j*mp.meijerg([[],[]],[[0,1],[1/2]],x*x) + 2*eps*(abs(x)*mp.hyper([],[1/2,3/2],x*x)-mp.sqrt(mp.pi)*x*x*mp.hyper([],[3/2,2],x*x))
